...
describe("agent browser access", () => {
  const startSessionWith = (enableAgentBrowserAccess: boolean, threadId: ThreadId) =>
    Effect.gen(function* () {
      const issued: Array<{ threadId: ThreadId; preview: boolean }> = [];
      const codex = makeFakeCodexAdapter();
      const providerAdapterLayer = Layer.succeed(
        ProviderAdapterRegistry.ProviderAdapterRegistry,
        ...
        issueMcpCredential: (request) =>
          Effect.sync(() => {
            issued.push({
              ...
              preview: request.capabilities?.has("preview") ?? true,
            });
            return undefined;
          }),
      }).pipe(
  ...
  it.effect(
    "still attaches T3 MCP tools when browser access is off, without preview capability",
    () =>
      Effect.gen(function* () {
