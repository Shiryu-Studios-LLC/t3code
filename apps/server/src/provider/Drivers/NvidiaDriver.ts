import { NvidiaSettings, ProviderDriverKind, type ServerProvider } from "@t3tools/contracts";
...
import { makeNvidiaTextGeneration } from "../../textGeneration/NvidiaTextGeneration.ts";
import { ProviderDriverError } from "../Errors.ts";
import { makeNvidiaAdapter } from "../Layers/NvidiaAdapter.ts";
import {
  buildInitialNvidiaProviderSnapshot,
  checkNvidiaProviderStatus,
  enrichNvidiaSnapshot,
...
export const NvidiaDriver: ProviderDriver<NvidiaSettings, NvidiaDriverEnv> = {
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const adapter = yield* makeNvidiaAdapter(effectiveConfig, {
        environment: processEnv,
        ...(eventLoggers.native ? { nativeEventLogger: eventLoggers.native } : {}),
        instanceId,
      });
      const textGeneration = yield* makeNvidiaTextGeneration(effectiveConfig, processEnv);

      const checkProvider = checkNvidiaProviderStatus(effectiveConfig, processEnv).pipe(
      ...
      const snapshot = yield* makeManagedServerProvider<ProviderSnapshotSettings<NvidiaSettings>>({
        ...
        initialSnapshot: (settings) =>
          buildInitialNvidiaProviderSnapshot(settings.provider).pipe(Effect.map(stampIdentity)),
        checkProvider,
